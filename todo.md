...
- [x] FR-AUTH-06: session idle timeout - fixed 2026-08-23, 15 minutes (user's deliberate call, deviates from the SRS's stated 30 - matches this project's existing `PIN_LOCKOUT_MINUTES`/`EMAIL_CHANGE_CODE_LOCKOUT_MINUTES` convention instead). Two settings, no custom code: `SESSION_COOKIE_AGE = 15*60` plus `SESSION_SAVE_EVERY_REQUEST = True` (makes it a sliding idle timeout - each request resets the clock - rather than a fixed 15-minute session regardless of activity). No model/migration needed, Django's own session-expiry mechanism handles it. Absorbed the "Session timeout / auto-logout for shared/cybercafe use" brainstorm item, which was the same feature under a different name - removed from Brainstormed rather than left duplicated. 3 new tests in `accounts/tests.py:SessionExpiryTests`, full suite 233 passing.
- [x] `DEFAULT_PASSWORD` shared-secret risk - fixed 2026-08-23, but not the originally-scoped "random per-account password" idea. User's own design instead: students no longer get any initial password at all (`create_student_account` calls `set_unusable_password()`), and can log in with just their username while `must_change_password=True` - `User.skips_first_login_password` (`accounts/models.py`), consulted only in `LenientUsernameBackend.authenticate()`. Safe because `ForcePasswordChangeMiddleware` already locks such an account to 4 URLs until the PIN emailed to their registered address is verified - that PIN check was already a stronger identity check than the old shared password, which was actively harmful (guessable username + publicly-displayed shared password let anyone log into an unclaimed account before its real owner). Staff were initially left on the shared `DEFAULT_PASSWORD` scheme, since they have no PIN-equivalent identity check - see the staff setup-link entry below, which closed that gap the same session. `students.services.reset_student_first_login` is the student-specific counterpart to `accounts.services.send_staff_setup_link`. Login page tip reworded to stay role-agnostic ("Just enter your username"), consistent with the project's earlier decision against the login page revealing it serves two audiences differently - also stops broadcasting the shared staff password on a public page. 5 new tests in `accounts/tests.py:PasswordlessFirstLoginTests`; fixed several existing test fixtures across `accounts/courses/students/results` that had force-bypassed `must_change_password` without ever setting a real password (previously relied on `DEFAULT_PASSWORD` doubling as a real one). Full suite 238 passing.
- [x] LEVEL_CHOICES/500-level cap - turned out to already be stale, not a real bug. Checked the actual code (2026-08-23): `LEVEL_CHOICES` already includes 600, and `StudentProfile.current_level` already caps at `self.department.duration_years * 100` (per-department, not a fixed 500) - fixed in commits `03f90b9` and `fe4c32c`, both predating this session. No hardcoded 500 cap found anywhere else in the codebase. Same staleness pattern already flagged for `feedback_flag_new_django_app_boundaries.md` - the todo item just never got marked resolved when the underlying fix landed.
- [x] Staff `DEFAULT_PASSWORD` shared-secret risk - fixed 2026-08-24, closing the gap the earlier student-side fix (above) deliberately left open. Staff accounts now get no password at all (`assign_staff_identity` calls `set_unusable_password()`); an Admin sends a one-time emailed setup link from Manage Staff whenever they want to invite that person, instead of it going out automatically at creation or ever showing a shared password on screen. `accounts.services.send_staff_setup_link` reuses Django's own password-reset token mechanism directly (`default_token_generator` + `urlsafe_base64_encode`) rather than going through `PasswordResetForm`, since that form's `get_users()` would silently exclude any account with no usable password. Reuses the existing forgot-password confirm view/URL as the landing page. "Reset Password" button in Manage Staff renamed to "Send setup link"; resending auto-invalidates the previous link. Staff never get a blank-password login attempt at all, unlike the student flow - the emailed link is the only door in. 5 new tests in `accounts/tests.py:StaffSetupLinkTests`, full suite 245 passing.
- [x] Three bugs found in manual testing of the above, all fixed 2026-08-24: (1) blank login password silently kept a stale session logged in, or crashed with `AttributeError: 'AnonymousUser' object has no attribute '_meta'` - `LoginForm` made password non-required for the passwordless student flow, but stock `AuthenticationForm.clean()` only calls `authenticate()` when password is truthy, so a blank password never reached the backend at all; `LoginForm.clean()` now overrides that gate. (2) Django's console email backend soft-wraps long lines (RFC 5322, 76 chars) with a trailing `=`, silently breaking any hand-copied link (forgot-password/staff-setup) split across two lines - fixed with a console-only backend subclass (`lu_sims/mail_backends.py`) requesting a much higher line-length policy; scoped to console only, doesn't touch real SMTP wire format. (3) Setting a new password via an emailed link (forgot-password or staff setup) left the user anonymous, forcing a redundant re-login - unlike the student first-login flow, which already stays logged in via `update_session_auth_hash()`. `ForgotPasswordConfirmView.form_valid()` now calls `login()` directly after the password is set.
- [x] Bursar role had zero accounts - created one (2026-08-24, `LU-BS-26-0001`) via the app's own `assign_staff_identity`/`generate_staff_id` service functions, so all 7 roles (Admin/Dean/HOD/Registrar/Bursar/Lecturer/Student) now have at least one real account. Data only, not a code change - not git-tracked.
